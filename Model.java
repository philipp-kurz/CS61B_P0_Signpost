 ...
 *  in that sequence.
 *
 *  @author Philipp Kurz according to CS 61B skeleton
 */
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        _board = new Sq[_width][_height]; _solnNumToPlace = new Place[last + 1];
        for (int number = 1; number <= last; number += 1) {
            ...
            for (int x0 = 0; x0 < _width; x0 += 1) {
                ...
                for (int y0 = 0; y0 < _height; y0 += 1) {
                    ...
                    if (_solution[x0][y0] == number) {
                        _solnNumToPlace[number] = Place.pl(x0, y0);
                        for (int x1 = 0; x1 < _width; x1 += 1) {
                            if (squareAdded) {
                                break;
                            }
                            for (int y1 = 0; y1 < _height; y1 += 1) {
                                if (_solution[x1][y1] == number + 1
                                        || number == last) {
                                    int dir = 0;
                                    if (number != last) {
                                        dir = Place.dirOf(x0, y0, x1, y1);
                                    }
                                    int sequenceNum = 0;
                                    boolean fixed = false;
                                    int group = -1;
                                    ...
                                    _board[x0][y0] =
                                        new Sq(x0, y0, sequenceNum,
                                                fixed, dir, group);
                                    _allSquares.add(_board[x0][y0]);
                                    squareAdded = true; break;
                                }
                            }
                        }
                    }
    ...
    /** Setting successors and predecessors. */
    void setSuccPre() {
        _width = _solution.length;
        _height = _solution[0].length;
        int last = _width * _height;
        for (int x0 = 0; x0 < _width; x0 += 1) {
            for (int y0 = 0; y0 < _height; y0 += 1) {
                if (_board[x0][y0]._sequenceNum == last) {
                    continue;
                }
                _board[x0][y0]._successors
                    = _allSuccessors[x0][y0][_board[x0][y0]._dir];
            }
        }
        for (int x0 = 0; x0 < _width; x0 += 1) {
            for (int y0 = 0; y0 < _height; y0 += 1) {
                for (int x1 = 0; x1 < _width; x1 += 1) {
                    for (int y1 = 0; y1 < _height; y1 += 1) {
                        int dir = dirOf(x1, y1, x0, y0);
                        if (dir == _board[x1][y1]._dir && dir != 0) {
                            if (_board[x0][y0]._predecessors == null) {
                                _board[x0][y0]._predecessors = new PlaceList();
                            }
                            _board[x0][y0]._predecessors.add(_board[x1][y1].pl);
                        }
                    }
                }
            }
     ...
     *  // FIXME: Fill in the _successor, _predecessor,
     *  //        and _head fields of the
     *  //        copied Sq objects.*/
    Model(Model model) {
        ...
        for (int x0 = 0; x0 < _width; x0 += 1) {
            for (int y0 = 0; y0 < _height; y0 += 1) {
                Sq sq = model._board[x0][y0];
                _board[x0][y0] = new Sq(sq.x, sq.y, sq._sequenceNum,
                        sq._hasFixedNum, sq._dir, sq._group);
                _allSquares.add(_board[x0][y0]);
            }
        }
        for (int x0 = 0; x0 < _width; x0 += 1) {
            for (int y0 = 0; y0 < _height; y0 += 1) {
                ...
                if (sq._successor != null) {
                    _board[x0][y0]._successor
                        = _board[sq._successor.x][sq._successor.y];
                } else {
                    _board[x0][y0]._successor = null;
                }
                if (sq._predecessor != null) {
                    _board[x0][y0]._predecessor =
                        _board[sq._predecessor.x][sq._predecessor.y];
                } else {
                    _board[x0][y0]._predecessor = null;
                }
                _board[x0][y0]._head = _board[sq._head.x][sq._head.y];
            }
        }
        int last = _width * _height;
        for (int x0 = 0; x0 < _width; x0 += 1) {
    ...
    boolean autoconnect() {
        ...
        for (Sq sq : this) {
            if (sq._sequenceNum != 0 && sq._successor == null) {
                for (Sq sq2 : this) {
                    if (sq2._sequenceNum != sq._sequenceNum + 1) {
                        continue;
                    } else {
                        if (dirOf(sq.x, sq.y, sq2.x, sq2.y)
                            == sq._dir && sq.connectable(sq2)) {
                            changesMade = sq.connect(sq2);
                        }
                    }
                }
            } else {
    ...
    void solve() {
        ...
        for (int i = 1; i <= last; i += 1) {
            _board[_solnNumToPlace[i].x][_solnNumToPlace[i].y]._sequenceNum = i;
            if (i > 1) {
                _board[_solnNumToPlace[i].x]
                    [_solnNumToPlace[i].y]._predecessor =
                    _board[_solnNumToPlace[i - 1].x][_solnNumToPlace[i - 1].y];
            }
            if (i < last) {
                _board[_solnNumToPlace[i].x][_solnNumToPlace[i].y]._successor =
                    _board[_solnNumToPlace[i + 1].x][_solnNumToPlace[i + 1].y];
            }
            _board[_solnNumToPlace[i].x][_solnNumToPlace[i].y]._group = 0;
            _board[_solnNumToPlace[i].x][_solnNumToPlace[i].y]._head =
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            if (dirOf(x, y, s1.x, s1.y) != _dir) {
                return false;
            } else if (_successor != null || s1._predecessor != null) {
                return false;
        ...
        boolean connect(Sq s1) {
            ...
            if (_sequenceNum != 0 && s1._sequenceNum == 0) {
                ...
                int tmpSeqNum = _sequenceNum + 1;
                Sq tmpSq = s1;
                while (tmpSq != null) {
                    tmpSq._sequenceNum = tmpSeqNum;
                    tmpSeqNum += 1;
                    if (tmpSq._group != group) {
                        int a = 0;
                    } else {
                        tmpSq._group = 0;
                    }
                    tmpSq = tmpSq._successor;
                }
            } else if (_sequenceNum == 0 && s1._sequenceNum != 0) {
                int group = _group;
                if (group != -1) {
                ...
                int tmpSeqNum = s1._sequenceNum - 1;
                Sq tmpSq = this;
                ...
                while (tmpSq != null) {
                    ...
                    tmpSq = tmpSq._predecessor;
                }
            } else if (_group == -1 && s1._group == -1) {
                _group = s1._group = newGroup();
                s1._head = _head;
            } else if ((_group == -1 && s1._group != -1 && s1._group != 0)
                    || (_group != -1 && s1._group == -1 && _group != 0)) {
                _group = s1._group = joinGroups(_group, s1._group);
                s1._head = _head;
            } else if (_group != -1 && s1._group != -1 && _group != s1._group
                    && _group != 0 && s1._group != 0) {
                _group = s1._group = _head._group =
                        s1._head._group = joinGroups(_group, s1._group);
                s1._head = _head;
            }
            for (Sq tmpSq = this; tmpSq != null; tmpSq = tmpSq._successor) {
                tmpSq._head = _head;
            }
            return true;
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                int newGrp = newGroup();
                Sq tmpSq = this;
                for (; tmpSq != null; tmpSq = tmpSq._predecessor) {
                    if (tmpSq._hasFixedNum) {
                        fixed = true; break;
                    }
                }
                if (!fixed) {
                    if (_predecessor == null) {
                        ...
                        tmpSq = this;
                        while (tmpSq != null) {
                            tmpSq._sequenceNum = 0; tmpSq._group = newGrp;
                            tmpSq = tmpSq._predecessor;
                        }
                    }
                }
                fixed = false;
                for (tmpSq = next; tmpSq != null; tmpSq = tmpSq._successor) {
                    if (tmpSq._hasFixedNum) {
                        fixed = true; break;
                    }
                }
            ...
            for (Sq tmpSq = next; tmpSq != null; tmpSq = tmpSq._successor) {
                tmpSq._head = next;
            } }
