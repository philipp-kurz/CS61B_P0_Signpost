        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place succ : sq.successors()) {
                    Sq succSq = model.get(succ);
                    if (sq.connectable(succSq)) {
                        if (sq.sequenceNum() != 0
                                && succSq.sequenceNum() != 0) {
                            nFound = 1;
                            found = succSq;
                            break;
                        }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                ...
                for (Place pre : sq.predecessors()) {
                    Sq preSq = model.get(pre);
                    if (preSq.connectable(sq)) {
                        if (sq.sequenceNum() != 0 && preSq.sequenceNum() != 0) {
                            nFound = 1;
                            found = preSq;
                            break;
                        }
